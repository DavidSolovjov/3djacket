class Wizard3DJacket {
    ...
    getSceneIntersectedObjects(event) {
        this.mouse.x = ((event.clientX - document.getElementById("canvasModel").getBoundingClientRect().left) / document.getElementById("canvasModel").getBoundingClientRect().width) * 2 - 1;// this line i changed june 17
            ...
            for (let i = 0; i < intersects.length; i++) {
                if (this.objectarr.includes(intersects[i].object)) { /*enable the commented code below june */
                    ...
                    if (objname == "hoodie_collar" || objname == "retro_collar" || objname == "classic_collar") {
                        ...
                        if (document.querySelectorAll('#jacketCustomization.uk-open').length == 0) {
                            document.querySelectorAll('button[data-uk-toggle="target: #jacketCustomization"]')[0].click();
                        }
                        setTimeout(function () {
                            let tarDiv;
                            ...
                            document.querySelectorAll("#jacketCustomization > div")[0].scrollTop = tarDiv.offsetTop - 50;
                        }, 50);
                    }
    ...
    onTouchMove(event) {
        ...
        if (intersects.length > 0) {
            this.objectarr = this.objectGroup.children.slice();
            this.objectarr.splice(0, -5);// this line i changed june 17
